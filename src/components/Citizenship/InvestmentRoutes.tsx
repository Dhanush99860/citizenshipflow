...
export default function InvestmentRoutes({ routes, className = "" }: Props) {
  ...
  const cheapest = minBy(routes, (r) => (isFiniteNum(r.minInvestment) ? (r.minInvestment as number) : Infinity));
  const fastest  = minBy(routes, (r) => (isFiniteNum(r.timeline) ? (r.timeline as number) : Infinity));
  ...
  return (
    ...
    >
      ...
      <header className="relative mb-4 md:mb-5">
        ...
        <h2 id={`${id}-title`} className="mt-2 text-lg md:text-xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-100">
          Investment routes
        </h2>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {cheapest?.minInvestment != null ? (
            <Chip label="From" value={fmtCurrency(cheapest.minInvestment!, cheapest.currency)} />
          ) : null}
          {fastest?.timeline != null ? <Chip label="Fastest" value={plural(fastest.timeline!, "mo")} /> : null}
        </div>
      </header>
          ...
          return (
            <li key={`${r.title}-${idx}`} className="group">
              ...
              >
                ...
                <div aria-hidden className={`pointer-events-none absolute -right-10 -top-10 h-24 w-24 rounded-full ${accent.arc} opacity-10 blur-2xl`} />
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100" itemProp="category">
                      {r.title}
                    </h3>
                ...
                <dl className="mt-3 space-y-1.5 text-sm">
                  <Row label="From" value={isFiniteNum(r.minInvestment) ? fmtCurrency(r.minInvestment!, r.currency) : "—"} />
                  <Row label="Fastest timeline" value={isFiniteNum(r.timeline) ? plural(r.timeline!, "mo") : "—"} />
                  {isFiniteNum(r.count) ? <Row label="Programs" value={String(r.count)} /> : null}
                </dl>
                ...
                {isFiniteNum(r.minInvestment) ? (
                  <>
                    ...
                    {r.currency ? <meta itemProp="priceCurrency" content={r.currency.toUpperCase()} /> : null}
      ...
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(toItemListJsonLd(routes)) }}
      />
    </section>
  );
}
  ...
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 print:hidden">
      ...
      <svg className="absolute inset-0 h-full w-full opacity-[0.03] dark:opacity-[0.05]" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="ir-grid" width="24" height="24" patternUnits="userSpaceOnUse">
...
function iconFor(title: string) {
  ...
  if (/\bdonation|contribution|national fund/.test(t)) return <CoinIcon className="h-4 w-4" />;
  if (/\b(real ?estate|property|home|house)/.test(t)) return <HomeIcon className="h-4 w-4" />;
  if (/\bbond|securit/.test(t)) return <BondIcon className="h-4 w-4" />;
  if (/\b(business|enterprise|startup|company|investment fund)/.test(t)) return <BriefcaseIcon className="h-4 w-4" />;
...
function toItemListJsonLd(routes: RouteItem[]) {
  const itemListElement = routes.map((r, i) => {
    ...
    offer.description = [
      isFiniteNum(r.timeline) ? `Fastest timeline: ${plural(r.timeline!, "mo")}` : null,
