...
/**
 * AboutSection — Introduces Varun Singh with narrative focus and dual media support.
 *
 * If an image source exists, it is displayed by default. Otherwise, a video source
 * (if available) will be shown instead. Video controls appear only when a video is present.
 */
export default function AboutSection() {
  const [isPlaying, setIsPlaying] = useState(true);
  const videoRef = useRef<HTMLVideoElement | null>(null);

  // Set these variables to the actual image or video file names
  const imageSrc = "/images/avtar/varun-singh-md-xiphias.jpg"; // replace with actual image path
  const videoSrc = "/images/personal/video/sample.mp4"; // replace with actual video path

  const hasImage = Boolean(imageSrc);
  const hasVideo = Boolean(!hasImage && videoSrc); // show video if image is absent
    ...
    >
      ...
      <div className="relative max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          {/* RIGHT CONTENT – Media (Image or Video) */}
          <div className="relative order-first md:order-none rounded-3xl overflow-hidden shadow-2xl aspect-[16/9] md:aspect-auto md:h-full border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
            ...
            {/* Media: show image if provided, otherwise show video */}
            {hasImage ? (
