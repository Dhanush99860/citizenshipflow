...
/* --------------------------------- utils --------------------------------- */
function useDebouncedCallback<T extends (...args: any[]) => void>(cb: T, delay = 350) {
  return useMemo(() => {
    let t: any;
...
export default function FiltersBar({
  ...
  const [kind, setKind] = useState<InsightKind | "">((initialKind as any) || "");
  ...
  const apply = useCallback(
    (next: { q?: string; kind?: string; country?: string; program?: string; tag?: string }) => {
      const params = new URLSearchParams(searchParams.toString());

      if (next.q !== undefined) (next.q ? params.set("q", next.q) : params.delete("q"));
      if (next.kind !== undefined) (next.kind ? params.set("kind", next.kind) : params.delete("kind"));
      if (next.country !== undefined) (next.country ? params.set("country", next.country) : params.delete("country"));
      if (next.program !== undefined) (next.program ? params.set("program", next.program) : params.delete("program"));
      if (next.tag !== undefined) (next.tag ? params.set("tag", next.tag) : params.delete("tag"));
  ...
  if (q) chips.push({ key: "q", label: `Keyword: ${q}`, onClear: () => apply({ q: "" }) });
  if (kind) chips.push({ key: "kind", label: `Type: ${kind}`, onClear: () => apply({ kind: "" }) });
  if (country) chips.push({ key: "country", label: `Country: ${country}`, onClear: () => apply({ country: "" }) });
  if (program) chips.push({ key: "program", label: `Program: ${program}`, onClear: () => apply({ program: "" }) });
  if (tag) chips.push({ key: "tag", label: `Tag: ${tag}`, onClear: () => apply({ tag: "" }) });
  ...
  return (
    <>
      ...
      {/* Mobile: one compact bar + bottom sheet for the rest */}
      <section
      ...
      >
        ...
        {chips.length > 0 && (
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            {chips.length > 3 && (
              <span className="text-[12px] text-neutral-500">+{chips.length - 3} more</span>
            )}
          </div>
        )}
      </section>
