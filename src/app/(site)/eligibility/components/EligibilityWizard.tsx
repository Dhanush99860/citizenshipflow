...
type Schema = { title: string; steps: Step[]; scoring: Rule[]; bands: Band[]; recommendations: Recommendation[] };
...
export default function EligibilityWizard({ schema }: { schema: Schema }) {
  ...
  const setField = (id: string, v: any) => setValues((s) => ({ ...s, [id]: v }));
  ...
  if (stepIdx === schema.steps.length) {
    const score = schema.scoring.reduce((acc, r) => {
      const c = r.if; let ok = true;
      if (c.age_lt !== undefined) ok &&= (values.age ?? Infinity) < c.age_lt;
      if (c.english_gte !== undefined) ok &&= (values.english ?? 0) >= c.english_gte;
      if (c.liquid_funds_gte !== undefined) ok &&= (values.liquid_funds ?? 0) >= c.liquid_funds_gte;
      if (c.education_in !== undefined) ok &&= c.education_in.includes(values.education);
      if (c.willing_invest !== undefined) ok &&= Boolean(values.willing_invest) === c.willing_invest;
      return ok ? acc + r.add : acc;
    }, 0);
    const band = schema.bands.sort((a, b) => b.min - a.min).find((b) => score >= b.min)!;
    const recos =
      schema.recommendations.sort((a, b) => b.when_min - a.when_min).find((r) => score >= r.when_min)?.programs ?? [];
    return (
      <div className="space-y-4">
        <div className="text-2xl font-semibold">Your score: {score}</div>
        <div className="text-lg">Result: <span className="capitalize">{band.label}</span></div>
        {recos.length > 0 && (
          <div className="space-y-2">
            ...
            <ul className="list-disc pl-6">
              {recos.map(([v, c, p]) => (
                <li key={`${v}/${c}/${p}`}>
                  <a className="text-blue-600 hover:underline" href={`/${v}/${c}/${p}`}>{`${v} / ${c} / ${p}`}</a>
                </li>
              ))}
            </ul>
          </div>
        )}
        <button className="rounded-xl border px-4 py-2" onClick={() => setStepIdx(0)}>Start again</button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {stepIdx > 0 && <button className="rounded-xl border px-4 py-2" onClick={prev}>Back</button>}
        <button className="rounded-xl border px-4 py-2" onClick={next}>Continue</button>
      </div>
    </div>
  );
}
