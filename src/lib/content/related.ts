...
export function getRelated(current: AnyDoc, limit = 6) {
  ...
  const verts = new Set(current.kind === 'program' ? [current.vertical] : current.verticals ?? []);
  const countries = new Set(current.kind === 'program' ? [current.country] : current.countries ?? []);
  const programs = new Set(current.kind === 'program' ? [current.program] : current.programs ?? []);
